...
def _create_tf_record_from_coco_annotations(annotations_file, image_dir, output_path, include_masks):
    ...
    for idx, one_line in enumerate(images[1:]):
        ...
        category_of_eachbox = [3 for i in range(len(annotations_list))]
...
def main(_):
  ...
  train_output_path = os.path.join(FLAGS.output_dir, 'DC_car_train.record')
  val_output_path = os.path.join(FLAGS.output_dir, 'DC_car_val.record')
